...
def create_dfa_cnf(nodes, edges, init_state, n, alphabet, graph, pos, negs):
    ...
    sub_clauses = [[0-nodes[final_node, q], edges[q,-1,-1]] for q in range(n) for final_node in pos]
    
    # reject_node = search_vertex(graph, init_state, negs[0])
    # print("reject node: " + str(reject_node))
    sub_clauses = [[0-nodes[reject_node, q], -edges[q,-1,-1]] for q in range(n) for reject_node in negs] + sub_clauses
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Minimise automata with dont care words')
