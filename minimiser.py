...
def create_variables(n, alphabet, nb_states):
...
def create_dfa_cnf(nodes, edges, input_sdfa, n, alphabet):
...
def create_cnf(nodes, edges, parents, t_aux, m_aux, input_sdfa
               , n, alphabet, nobfs, safety):
    clauses = create_dfa_cnf(nodes, edges, input_sdfa,
                             n, alphabet)
    if not nobfs:
...
def solve(
    ...
    nodes, edges, parents, t_aux, m_aux = create_variables(n, alphabet, input_sdfa.num_states)
...
def main(infile, outfile, sat, lbound, ubound, nobfs, safety):
    
    input_sdfa = SDFA.sdfa()
    
    input_sdfa.load(infile)
    ...
    while n <= max_bound:
        ...
        res, dfa = solve(sat, n, alphabet, input_sdfa, nobfs, safety)
        if res:
            print("Output to " + outfile)
            write_dot(dfa, outfile)
            break
        else:
            n += 1
    end_time = time.time()
...
if __name__ == '__main__':
    ...
    main(infile=args.f, outfile=args.out, sat=args.solver
