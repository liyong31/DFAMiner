...
### Transferring data between the container and the host
￼
The data generated by the dfaminer container run in interactive mode exists only inside the container. 
To make such data available in the host machine, Docker provides a command to copy data to and from the container.
To copy data generated DFAMiner, use the following command in the host machine while the `dfaminer_container` container is running:
```bash
docker container cp dfaminer_container:/home/ubuntu/output .
```
￼
Transferring data to the running dfaminer container is only needed in case one needs to provide custom input files.
This can be achieved by one of the following commands:
```bash
docker container cp localfiles dfaminer_container:/home/ubuntu/inputs
```
The command copies the host local files `localfiles` into the already existing `inputs` directory.

The `inputs` and `outputs` directories could also be bind mounted to local host directories; see the [Docker documentation](https://docs.docker.com/engine/storage/bind-mounts/) about the mount and volume options for more information about how to use them.

In case the `inputs` or `outputs` directories are used, adapt the paths in the usage instructions below accordingly.

## Usage

For usage, type <b>`python3 dfaminer.py --help`</b> while the `dfaminer` virtual environment is active, or inside the docker container; this will produce the following output:
...
```
within the `dfaminer` virtual environment or inside the dfaminer container.
