...
class generator:
    ...
    def get_membership(num_colors, word):
        ...
        for i, color in enumerate(word):
            ...
            lft, rgt = positions[color]
            is_even = generator.is_even_loop(word, lft, rgt)
    ...
    def get_all_data(num_colors, length):
        ...
        pos = list(filter(lambda x: generator.get_membership(num_colors, x) == 1, perms))
        negs = list(filter(lambda x: generator.get_membership(num_colors, x) == -1, perms))
        doncares = list(filter(lambda x: generator.get_membership(num_colors, x) == 0, perms))
        all_valid = list(filter(lambda x: generator.get_membership(num_colors, x) != 0, perms))
        all_valid = [ (x, generator.get_membership(num_colors, x)) for x in all_valid]
        print(all_valid)
        return (pos, negs, all_valid, doncares)            
